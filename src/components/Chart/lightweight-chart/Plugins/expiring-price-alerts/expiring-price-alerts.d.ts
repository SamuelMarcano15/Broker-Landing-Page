...
declare class ExpiringPriceAlertsPaneRenderer implements ISeriesPrimitivePaneRenderer {
	_data: RendererDataItem[];
	draw(target: CanvasRenderingTarget2D): void;
	update(data: RendererDataItem[]): void;
}
declare class ExpiringPriceAlertsPaneView implements ISeriesPrimitivePaneView {
...
declare class ExpiringAlertPrimitive extends PluginBase {
	_source: ExpiringPriceAlerts;
...
export declare class ExpiringPriceAlerts implements IExpiringPriceAlerts {
	_options: ExpiringPriceAlertsOptions;
	_chart: IChartApi | null;
	_series: ISeriesApi<keyof SeriesOptionsMap>;
	_primitive: ExpiringAlertPrimitive;
	_whitespaceSeriesStart: number | null;
	_whitespaceSeriesEnd: number | null;
	_whitespaceSeries: ISeriesApi<"Line">;
	_alerts: Map<string, ExpiringPriceAlert>;
	_dataChangedHandler: (scope: DataChangedScope) => void;
	constructor(series: ISeriesApi<keyof SeriesOptionsMap>, options: Partial<ExpiringPriceAlertsOptions>);
	destroy(): void;
	alerts(): Map<string, ExpiringPriceAlert>;
	chart(): IChartApi | null;
	...
	addExpiringAlert(price: number, startDate: number, endDate: number, parameters: ExpiringPriceAlertParameters): string;
	removeExpiringAlert(id: string): void;
	toggleCrossed(id: string): void;
	checkExpired(time: number): void;
	_lastValue: number | undefined;
	checkedCrossed(point: LineData | WhitespaceData): void;
	_update(): void;
